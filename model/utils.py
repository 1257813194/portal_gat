...
def preprocess_datasets(adata_list, # list of anndata to be integrated
                        hvg_num=4000, # number of highly variable genes for each anndat):
                        cutoff=6):
    ...
    for i, adata in enumerate(adata_list):
        ...
        if i == 0:
            ...
            hvg_total = list(set(hvg_total) & set(hvg))
    if len(hvg_total) < 100:
        ...
        print('{} highly variable genes'.format(len(hvg_total)))
    ...
    data_new_list=[]
    for i,adata in enumerate(adata_list):
        adata_new=anndata.AnnData(lowdim_list[i])
        adata_new.obsm['spatial']=adata.obsm['spatial']
        Cal_Spatial_Net(adata_new,k_cutoff=cutoff,model="KNN")
        data_new_list.append(Transfer_pytorch_Data(adata_new))
    return data_new_list


def integrate_datasets(data_list,
                       lambda_cos=20.0,
                       training_steps=2000,
                       add=0
                       ):
    print("Incrementally integrating %d datasets..." % len(data_list))
    B_space=[]
    latent=[]
    for i in range(len(data_list) - 1):
        print("Integrating the %d-th dataset to the 1-st dataset..." % (i + 2))
        model = Model(lambdacos=lambda_cos,training_steps=training_steps)
        if i == 0:
            emb_A = data_list[0]
        else:
            emb_A = emb_total
        emb_B = data_list[i + 1]
        model.train(transformed_data=[emb_A,emb_B])
        x=torch.tensor(model.B_space)
        edge_index=torch.cat([emb_A.edge_index,emb_B.edge_index+emb_A.num_nodes],axis=1)
        emb_total=Data(x=x,edge_index=edge_index)
        B_space.append(emb_total)
        latent.append(model.latent)
        lambda_cos+=add
    return latent, B_space
    
def integrate_datasets_revise(new_data_list,
                       lambda_cos=20.0,
                       training_steps=1500,
                       warmup=5,
                       ):
    print("Incrementally integrating %d datasets..." % len(new_data_list))
    B_space=[]
    latent=[]
    z=[]
    id_list=[np.eye(len(new_data_list))[i] for i in range(len(new_data_list))]
    for i in range(len(new_data_list) - 1):
        print("Integrating the %d-th dataset to the 1-st dataset..." % (i + 2))
        model = Model(lambdacos=lambda_cos,training_steps=training_steps,warmup=warmup)
        if i == 0:
            emb_A = new_data_list[0]
            emb_B = new_data_list[i + 1]
            id_A = id_list[i]
            id_B = id_list[i+1]
            model.train(transformed_data=[emb_A,emb_B],id_A=id_A,id_B=id_B)
        else:
            emb_A = emb_total
            emb_B = new_data_list[i + 1]
            id_A = id_A+id_list[i]
            id_B = id_list[i+1]
            model.train(transformed_data=[emb_A,emb_B],id_A=id_A,id_B=id_B,graph_dict=graph)
        graph=model.GRAPH.state_dict()
        x=torch.tensor(model.B_space)
        edge_index=torch.cat([emb_A.edge_index,emb_B.edge_index+emb_A.num_nodes],axis=1)
        emb_total=Data(x=x,edge_index=edge_index)
        B_space.append(emb_total)
    ...
    for i in range(len(new_data_list)):
            z.append(model.GRAPH(new_data_list[i].x.cuda(),new_data_list[i].edge_index.cuda(),id_list[i]).detach().cpu().numpy())
    return latent, B_space, np.concatenate(z)
